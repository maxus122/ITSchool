...
int main(){
    ...
    int x1=-75; int y1=400; double shoesFines=0; double pantsFines=0; bool  mood=1; bool eyesFines=1; bool hairFines=1; bool reverseColor=0; bool legsFines=1; bool bodyFines=1; double scaleFines=1;
    //drawFines(x1, y1, shoesFines,  pantsFines, mood, eyesFines, hairFines, reverseColor, legsFines, bodyFines, scaleFines);

    int x2=-200; int y2=450; double eyeJake =1; int colorJake=0; bool noseJake=1; bool mouthJake=0; bool armsJake=1; double headJake=0; bool earsJake=1; bool armsUpJake=0; double scaleJake=1;
    //drawJake(x2, y2, eyeJake, colorJake, noseJake, mouthJake, armsJake, headJake, earsJake, armsUpJake, scaleJake);

    double x3=-400; double y3=500; double eyeDagget=0; double teethDagget=0; bool hairDagget=1; double handsDagget=0; double legsDagget=0; int colorDagget=0; bool noseDagget=1; bool legsHandsDagget=1; double scaleDagget=1;
    ...
    int x4=910; int y4=85; bool capMario=0; bool mustacheMario=1; int hairMario=0; string eyesMario= "dark"; bool handsUpMario=0; int bootsMario=0; bool pantsMario=0; bool headMario=1; double scaleMario=0.2;
    //drawMario(x4, y4, capMario, mustacheMario, hairMario, eyesMario, handsUpMario, bootsMario, pantsMario, headMario, scaleMario);

    int x5=900; int y5=490; int mouthCJ =0; bool colorCJ = 0; string colorShirtCJ= "white"; bool legsCJ=1; bool faceCJ=1;  double eyesCJ=0; bool hairCJ=1; bool bodyCJ=1; double scaleCJ=0.2;
    //drawCJ(x5, y5, mouthCJ, colorCJ, colorShirtCJ, legsCJ, faceCJ, eyesCJ, hairCJ, bodyCJ, scaleCJ);

    int x6=-425; int y6=400; double eyeMike=1; double hornMike=0; bool doubleEyesMike=0; bool mouthOpenMike=0; bool colorMike=0; bool doubleFingersLegs=1; double scaleMike= 0.6;
    //drawMike(x6, y6, eyeMike, hornMike, doubleEyesMike, mouthOpenMike, colorMike, doubleFingersLegs, scaleMike);

    int xB=0; int yB=0; int xB2=1249; int yB2=0;
    drawBackground(xB, yB);

    int x=910; int y=240; double scalePipe=0.3;

    int time=0; bool reverseY=1; bool reverseX=1; int xD; int yD; double scaleDialog; bool MCJ; bool reverseLogic=0;
    while(time<100){
        txClear();
        drawBackground(xB, yB);
        x1=x1+9;
    ...
    while(time<120){
        txClear();
        drawBackground(xB, yB);
        xD=325; yD=425; scaleDialog=1;
        drawDialog(xD, yD, scaleDialog);
        txTextOut(xD+35, yD-85, "Здарова, парни! Вы");
        txTextOut(xD+35, yD-55, "СиДжея не видели?");
        mouthOpenMike=!mouthOpenMike;
        drawFines(x1, y1, shoesFines,  pantsFines, mood, eyesFines, hairFines, reverseColor, legsFines, bodyFines, scaleFines);
        drawJake(x2, y2, eyeJake, colorJake, noseJake, mouthJake, armsJake, headJake, earsJake, armsUpJake, scaleJake);
        drawDagget(x3, y3, eyeDagget, teethDagget, hairDagget, handsDagget, legsDagget, colorDagget, noseDagget, legsHandsDagget, scaleDagget);
        drawMike(x6, y6, eyeMike, hornMike, doubleEyesMike, mouthOpenMike, colorMike, doubleFingersLegs, scaleMike);
    ...
    while(time<400){
        ...
        if (time<250){
            ...
            xD=1050; yD=360; scaleDialog=0.8;
            drawDialog(xD, yD, scaleDialog);
            txTextOut(xD+35, yD-65, "Кто меня звал?");
            txTextOut(xD+50, yD-45, "Я здесь!");
        }
        if (time>250){
            xD=880; yD=410; scaleDialog=1.4;
            drawDialog(xD, yD, scaleDialog);
            txTextOut(xD+45, yD-125, "Меня больше смущает");
            txTextOut(xD+40, yD-100, "отсутствие и снега, и Марио!");
            txTextOut(xD+50, yD-75, "Завтра уже декабрь!");
            if(time%2==0){
                mood=!mood;
            }
        }
    ...
    while(time<550){
        ...
        drawMike(x6, y6, eyeMike, hornMike, doubleEyesMike, mouthOpenMike, colorMike, doubleFingersLegs, scaleMike);
        if (time<450){
            xD=760; yD=440; scaleDialog=0.85;
            drawDialog(xD, yD, scaleDialog);
            txTextOut(xD+25, yD-50, "Надо подумать...");
            if (time<410){
                if(time%4==0){
                    mouthJake=!mouthJake;
                }
        ...
        if (time>450){
            xD=760; yD=440; scaleDialog=2;
            drawDialog(xD, yD, scaleDialog);
            txTextOut(xD+46, yD-165, "Слушайте, я вначале осени слышал, как");
            txTextOut(xD+45, yD-135, "Марио говорил о трубе на Северный полюс!");
            txTextOut(xD+47, yD-105, "Мне кажется он что-то там натворил...");
            txTextOut(xD+52, yD-75, "Пошлите! Нужно спасать его.");
            if(time%4==0){
            mouthJake=!mouthJake;
            }
        }
    ...
    HDC weeks = txLoadImage("Resources\\Images\\weeks.bmp");
    txBitBlt (txDC(), 0, 0, 1250, 800, weeks, 0, 0);
    txSleep(4000);
    txClear();
    txDeleteDC (weeks);
    HDC house3 = txLoadImage("Resources\\Images\\house3.bmp");
    x1=470;
    x2=260;
    x3=90;
    x5=570;
    ...
    while(time<772){
        ...
        //HDC house3 = txLoadImage("Resources\\Images\\house3.bmp");
        txBitBlt (txDC(), 0, 0, 1250, 800, house3, 0, 0);
        xD=1010; yD=85; scaleDialog=0.8;
        drawDialog(xD, yD, scaleDialog);
        txTextOut(xD+43, yD-65, "Не понял.");
        txTextOut(xD+40, yD-50, "СНЕЖОК! СЮДА!");
        drawMario(x4, y4, capMario, mustacheMario, hairMario, eyesMario, handsUpMario, bootsMario, pantsMario, headMario, scaleMario);
        drawPipe(x, y, scalePipe);
        drawJake(x2, y2, eyeJake, colorJake, noseJake, mouthJake, armsJake, headJake, earsJake, armsUpJake, scaleJake);
        drawFines(x1, y1, shoesFines,  pantsFines, mood, eyesFines, hairFines, reverseColor, legsFines, bodyFines, scaleFines);
    ...
    int y52= 400; int x52=1300; double scaleCJ2=1; color=0; bool colorCJ2=1; string colorShirtCJ2="white";
    xD=210; yD=585; scaleDialog=0.4;
    drawDialog(xD, yD, scaleDialog);
    txTextOut(xD+46, yD-35, "Э...");
    xD=520; yD=585; scaleDialog=0.4;
    drawDialog(xD, yD, scaleDialog);
    txTextOut(xD+46, yD-35, "Что?");
    xD=310; yD=585; scaleDialog=0.4;
    drawDialog(xD, yD, scaleDialog);
    txTextOut(xD+46, yD-35, "Не понял.");
    xD=410; yD=585; scaleDialog=0.4;
    drawDialog(xD, yD, scaleDialog);
    txTextOut(xD+46, yD-35, "???");
    txSleep(2000);
    while(time<1000){
        txClear();
        if (time<950){
        ...
        if (color==20){
            colorShirtCJ2="orange";
        }
        if (color==30){
            colorShirtCJ2="yellow";
        }
        if (color==40){
            colorShirtCJ2="brown";
        }
        ...
        txBitBlt (txDC(), 0, 0, 1250, 800, house3, 0, 0);
        drawCJ(x52, y52, mouthCJ, colorCJ2, colorShirtCJ2, legsCJ, faceCJ, eyesCJ, hairCJ, bodyCJ, scaleCJ2);
        drawMario(x4, y4, capMario, mustacheMario, hairMario, eyesMario, handsUpMario, bootsMario, pantsMario, headMario, scaleMario);
        drawPipe(x, y, scalePipe);
        drawJake(x2, y2, eyeJake, colorJake, noseJake, mouthJake, armsJake, headJake, earsJake, armsUpJake, scaleJake);
        drawFines(x1, y1, shoesFines,  pantsFines, mood, eyesFines, hairFines, reverseColor, legsFines, bodyFines, scaleFines);
        ...
        xD=1010; yD=85; scaleDialog=0.7;
        drawDialog(xD, yD, scaleDialog);
        txTextOut(xD+46, yD-55, "АТАКУЙ ИХ!");
        txSleep(30);
        time++;
    }
    while(time<2200){
        color=rand()%60+100;
        txSetColor(RGB(color, color, color));
        txSetFillColor(RGB(color, color, color));
        txCircle(rand()%1250, rand()%800, rand()%100+15);
        time++;
        if(time>2000){
            txSleep(1);
        }
    ...
    txClear();
    xB=0; yB=0;
    x6=500; y6=400; scaleMike=1; colorMike=!colorMike; doubleEyesMike=!doubleEyesMike; hornMike=hornMike+2;
    drawBackground(xB, yB);
    drawMike(x6, y6, eyeMike, hornMike, doubleEyesMike, mouthOpenMike, colorMike, doubleFingersLegs, scaleMike);
    xD=600; yD=400; scaleDialog=0.9;
    drawDialog(xD, yD, scaleDialog);
    txTextOut(xD+43, yD-80, "Ух, емае!");
    txTextOut(xD+38, yD-65, "Долбаный порошок.");
...
void drawPipe(int x, int y, double scalePipe){
    ...
    txSetColor(TX_BLACK,6*scalePipe);
    txSetFillColor(RGB(170, 210, 15));
    txRectangle(x+0.01*scalePipe, y-160*scalePipe, x+270*scalePipe, y+0.01*scalePipe);
    txRectangle(x-20*scalePipe, y-290*scalePipe, x+290*scalePipe, y-150*scalePipe);

    txSetColor(RGB(10, 140, 0));
    txSetFillColor(RGB(10, 140, 0));

    txRectangle(x-17*scalePipe, y-265*scalePipe, x+43*scalePipe, y-255*scalePipe);
    txRectangle(x+23*scalePipe, y-265*scalePipe, x+43*scalePipe, y-154*scalePipe);

    txRectangle(x+100*scalePipe, y-265*scalePipe, x+113*scalePipe, y-154*scalePipe);
    txRectangle(x+100*scalePipe, y-265*scalePipe, x+286*scalePipe, y-255*scalePipe);
    txRectangle(x+140*scalePipe, y-265*scalePipe, x+240*scalePipe, y-154*scalePipe);

    txRectangle(x+43*scalePipe, y-148*scalePipe, x+63*scalePipe, y-4*scalePipe);
    txRectangle(x+110*scalePipe, y-148*scalePipe, x+123*scalePipe, y-4*scalePipe);
    txRectangle(x+150*scalePipe, y-148*scalePipe, x+230*scalePipe, y-4*scalePipe);
}
